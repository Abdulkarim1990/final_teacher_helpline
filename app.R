...
insert_ticket <- function(con, region_id, channel_id, teacher_name, teacher_phone, teacher_staff_id,
  ...
  tryCatch({
    ...
    # Let the DB trigger (set_ticket_defaults) auto-generate the case_code
    query <- paste0("
      INSERT INTO tickets (
    ...
    if (rows_affected > 0) {
      ...
      case_code <- tryCatch({
        result <- dbGetQuery(con, "SELECT case_code FROM tickets WHERE ticket_id = LAST_INSERT_ID()")
        if (nrow(result) > 0) result$case_code[1] else "Unknown"
