...
insert_ticket <- function(con, region_id, channel_id, teacher_name, teacher_phone, teacher_staff_id,
                          ...
                          entry_mode = "full", quick_outcome = NULL, status = "New", created_by_user_id = NULL) {
  if (is.null(con)) {
  ...
  tryCatch({
    ...
    # case_code is omitted — the DB trigger (set_ticket_defaults) auto-generates it
    query <- paste0("
      INSERT INTO tickets (
        ...
        entry_mode, quick_outcome, created_by_user_id, resolved_at, created_at
    ...
    rows_affected <- dbExecute(con, query, params = list(
      region_id, channel_id, teacher_name, teacher_phone, teacher_staff_id,
      school_name, district, category_id, subcategory_id, priority, status, summary, description,
      entry_mode, quick_outcome, created_by_user_id
    ))

    if (rows_affected > 0) {
      # Retrieve the case_code generated by the DB trigger
      case_code_row <- dbGetQuery(con, "SELECT case_code FROM tickets WHERE ticket_id = LAST_INSERT_ID()")
      case_code <- if (nrow(case_code_row) > 0) case_code_row$case_code[1] else "unknown"
